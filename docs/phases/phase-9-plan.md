...
- `fr_recover_ext_candidate_to_file` now supports regular-file recovery from direct, single-indirect, and double-indirect block pointers.
- recovery marks partial when unresolved tail requires triple-indirect handling or reads fail.

## Deferred in Phase 9

- ext inode-table-backed deleted metadata reconstruction.
- ext recovery/export coverage for triple-indirect trees, extents, and directory/symlink semantics.
