...
- `cargo test -p fr-ffi` passed (`33` tests, includes ext direct/single/double/triple-indirect recovery, extent-leaf recovery, and sparse/uninitialized extent zero-fill semantics).
...
- UI recovery flow now executes ext direct/single/double/triple-indirect and extents-tree recovery (including sparse zero-fill handling) for supported regular-file candidates and returns status `91` for unsupported candidates.
...
- Expand ext recovery semantics for non-regular inode types.
