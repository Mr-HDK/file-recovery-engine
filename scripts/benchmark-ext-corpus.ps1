...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
  ...
  $OutputPath = Join-Path $repoRoot "tools\benchmark-results\ext-corpus-$stamp.json"
}

$outputFullPath = [System.IO.Path]::GetFullPath($OutputPath)
New-Item -ItemType Directory -Force -Path ([System.IO.Path]::GetDirectoryName($outputFullPath)) | Out-Null

$projectPath = Join-Path $repoRoot "tools\benchmarks\ExtCorpusBench\ExtCorpusBench.csproj"
...
if ($MaxEntries -gt 0) {
  $runnerArgs += @("--max-entries", $MaxEntries.ToString())
}
